...
export default function StatisticsNeedsChart() {
  const [series, setSeries] = useState([
    { name: "Needs", data: Array(12).fill(0) }
  ]);

  useEffect(() => {
    async function fetchData() {
      const amounts = await getUserAmounts();
      const monthlyNeeds = Array(12).fill(0);
      amounts.forEach((entry) => {
        const month = new Date(entry.timestamp).getMonth();
        monthlyNeeds[month] += entry.needs;
      });
      setSeries([{ name: "Investment", data: monthlyNeeds }]);
    }
    fetchData();
  }, []);
